...
router.get('/node/:nodeId', async (req, res) => {
    try {
        const node = await ToolRegistry_1.NodeRegistry.findOne({ nodeId: req.params.nodeId });
        if (!node)
...
router.get('/node/:nodeId/tools', async (req, res) => {
    try {
        const node = await ToolRegistry_1.NodeRegistry.findOne({ nodeId: req.params.nodeId }, { tools: 1, clientTypes: 1, isClientOrigin: 1, nodeName: 1 });
        if (!node)
            return res.status(404).json({ error: 'Node not found' });
        res.json({
...
router.get('/connections/:nodeId', async (req, res) => {
    try {
        const node = await ToolRegistry_1.NodeRegistry.findOne({ nodeId: req.params.nodeId }, { validConnections: 1, validChaos: 1, invalidChaos: 1 });
        if (!node)
            return res.status(404).json({ error: 'Node not found' });
        res.json(node);
    }
